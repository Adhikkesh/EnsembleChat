...
func (c *ChatClient) ConnectToServer(addr string) error {
	...
	c.serverAddr = addr
	fmt.Printf("%s[CONNECT] Connected to chat server at %s\n", c.prefix, addr)
	return nil
}
...
func (c *ChatClient) JoinRoomRPC(roomName string) error {
	...
	fmt.Printf("%s[JOIN] Joined room \"%s\" (server: %s)\n", c.prefix, roomName, c.serverAddr)
...
func (c *ChatClient) Disconnect() {
	if c.rpcClient != nil {
		c.rpcClient.Close()
		fmt.Printf("%s[DISCONNECT] Disconnected from server\n", c.prefix)
	}
}
